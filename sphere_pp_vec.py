...
with open('pacific_end_data.json', 'w') as f:
    json.dump(theta_vec_list, f)

inputs = np.array([spherical_to_euclidean(theta_vec[i])[0] for i in range(len(theta_vec))])
...
for i in range(50):
    ...
    theta_vec = np.linspace(.001, np.pi - .001, ngrids)
    phi_vec = np.linspace(.001, 2 * np.pi - .001, ngrids)

    X_sph = np.array([(x, y) for x in theta_vec for y in phi_vec])
    ...
    write_path = 'pacific_end_dens_est_radial_v' + str(i+20000) + '.json'
